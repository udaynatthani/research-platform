...
export default function AIAssistantPage() {
  ...
  const runAction = useCallback(async (action) => {
    if (!selectedPaper) { pushToast({ message: 'Select a paper first', type: 'info' }); return }
    setActionLoading(true); setActiveAction(action.key); setResult(null)
    try {
      const { method, url, body } = action.endpoint(selectedPaper, activeProject?.id)
        ...
        <div className="w-72 shrink-0 space-y-4">
          <div className="bg-white rounded-2xl border border-slate-100 p-4 space-y-3">
            ...
            <select value={selectedPaper} onChange={e => setSelectedPaper(e.target.value)}
              className="w-full text-sm rounded-lg border border-slate-200 p-2 focus:outline-none focus:ring-2 focus:ring-green-400">
          ...
          <div className="bg-white rounded-2xl border border-slate-100 p-4 space-y-2">
            ...
            {actions.map(action => (
              <button key={action.key} onClick={() => runAction(action)}
                ...
                className={`w-full flex items-center gap-3 p-3 rounded-xl text-sm font-medium text-left transition-colors border ${activeAction === action.key ? `bg-${action.color}-50 border-${action.color}-200 text-${action.color}-700` : 'bg-slate-50 border-slate-100 text-slate-700 hover:bg-green-50 hover:border-green-200'} disabled:opacity-50`}>
            ...
            <div className="flex-1 bg-white rounded-2xl border border-slate-100 p-5">
              {!result ? (
                <div className="flex flex-col items-center justify-center h-48 text-slate-300">
                  ...
                  <p className="text-sm">Select a paper and run an action</p>
                </div>
              ) : (
                <div className="prose prose-sm max-w-none">
                  <pre className="text-xs bg-slate-50 rounded-xl p-4 overflow-auto whitespace-pre-wrap text-slate-700">{typeof result === 'string' ? result : JSON.stringify(result, null, 2)}</pre>
                </div>
              )}
            </div>
          ) : (
            <div className="flex-1 flex flex-col bg-white rounded-2xl border border-slate-100 overflow-hidden" style={{ height: 'calc(100vh - 330px)' }}>
              <div className="flex-1 overflow-y-auto p-4 space-y-4">
                ...
                {messages.map((m, i) => (
                  <div key={i} className={`flex gap-3 ${m.role === 'user' ? 'flex-row-reverse' : ''}`}>
                    ...
                    <div className={`max-w-lg rounded-2xl px-4 py-3 text-sm ${m.role === 'user' ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-800'}`}>
                      {m.content}
                    </div>
                  </div>
                ))}
              ...
              <div className="p-3 border-t border-slate-100 flex gap-2">
                <input value={chatInput} onChange={e => setChatInput(e.target.value)}
                  ...
                  className="flex-1 text-sm rounded-xl border border-slate-200 px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-green-400 disabled:opacity-50" />
